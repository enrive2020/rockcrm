...
| Отменил преподаватель | в отработки / не списывается | в отработки |
...
**Почему отмена преподавателем даёт отработку, а не просто «не списывается».**
Одного несписанного занятия мало: в месячном абонементе слоты расписаны
по неделям, и пропавший урок ученик физически не отходит внутри срока —
занятие останется на балансе и сгорит вместе с ним. Отработка — это право
на внеплановый слот, и когда школа сама отменила урок, оно тем более
уместно, чем при отмене по инициативе ученика.
