...
_CONSTRAINT_ERRORS: dict[str, tuple[int, str, str]] = {
    ...
    # Имя ограничения до миграции 008: сплошной UNIQUE (lesson_id, student_id).
    # Оставлено, пока миграция не накатана на всех стендах — иначе база,
    # отставшая на один файл, отдавала бы администратору 500 вместо ответа.
    # Удалить вместе с последним неперенесённым стендом.
    ...
    # Периоды начисления не пересекаются: иначе одно занятие попало бы в две
    # выплаты, и сверить ведомость стало бы нечем.
    "payroll_period_tenant_id_period_excl": (
        409,
        "period_overlap",
        "Этот период пересекается с уже закрытым. Закрытый период "
        "не пересчитывается — правки идут корректировкой в следующий.",
    ),
    # --- воронка заявок ---
    "lead_lost_reason_ck": (
        422,
    ...
    "lead_external_uniq": (
        409,
        "duplicate_external_lead",
        "Заявка с таким external_id уже принята. Повторная доставка ничего не меняет.",
    ),
    "lesson_target_ck": (
        400,
        "bad_lesson_target",
        "Занятие ставится либо на ученика, либо на группу, либо на заявку — "
        "выберите что-то одно.",
    ),
    "notification_dedup": (
        409,
        "notification_duplicate",
        "Такое уведомление уже стоит в очереди.",
    ),
}
