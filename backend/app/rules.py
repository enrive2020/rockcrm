...
def makeup_expire_reason(expires_on: date) -> str:
    """Строка журнала о сгоревшей отработке.

    В тексте стоит срок из договора, а не день, когда до отработки дошло
    ночное задание. Родитель проверяет именно его: «сгорает через 30 дней
    от пропущенного урока» — дата, которую ему назвали при отмене занятия.
    Задание может опоздать на сутки после сбоя, и дата сгорания от этого
    двигаться не должна, иначе журнал разойдётся с тем, что обещали.
    """
    return f"Отработка сгорела: срок истёк {expires_on:%d.%m.%Y}"
